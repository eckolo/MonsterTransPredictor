...
namespace MonsterTransPredictor.Models.Application.Service
{
    ...
    {
        public static Skill[] GetSkillDetail(this List<int?> skillIdList, IEnumerable<Skill> skillList)
            => skillIdList?.Select(id => skillList?.SingleOrDefault(skill => skill.id == id)).ToArray();
